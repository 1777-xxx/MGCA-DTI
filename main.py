...
import numpy as np
from sklearn.metrics import roc_auc_score, average_precision_score, f1_score, accuracy_score, precision_score, recall_score, precision_recall_curve,confusion_matrix
import gc
...
def show_result(data_path, AUC_list, AUPR_list, F1_list, Accuracy_list, Precision_list, Recall_list,Specificity_list):
    ...
    with open("{}/result/TriDTI_results.txt".format(data_path), 'w') as f:
        ...
        f.write('Specificity(std):{:.4f}({:.4f})\n'.format(Specificity_mean, Specificity_std))
    ...
    print('Specificity(std):{:.4f}({:.4f})'.format(Specificity_mean, Specificity_std))
...
if __name__ == "__main__":
    ...
    result_path = f"./dataset/{DATASET_NAME}/result"
    os.makedirs(result_path, exist_ok=True)
        
    print(f"Load Dataset: {DATASET_NAME}")
    train_dataloader, valid_dataloader, test_dataloader, test_df, drug_sim_graph, prot_sim_graph = get_dataloaders(data_path, top_k_d, top_k_t, MAX_ATOM_NODES, MAX_DRUG_NODES, MAX_PROT_NODES, BATCH_SIZE, num_workers)
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        for batch in tqdm(train_dataloader):
            ...
            # loss = criterion(out.squeeze(1), label) + lambda_aux * aux_loss
            ####################################################################################################################################################
            loss = criterion(out, label) + lambda_aux * aux_loss
        ...
        print(f" Epoch : {epoch:4d}, Train Loss : {train_loss:.8f}")
        with torch.no_grad():
            model.eval()
            val_loss = 0.0
            total_out = list()
    ...
    with torch.no_grad():
        ...
        all_pred = (np.array(all_out) > 0.5).astype(int)

        tn, fp, fn, tp = confusion_matrix(all_labels, all_pred).ravel()
        ...
        fold_specificity_list.append(specificity)
    ...
    clear_memory()
show_result(data_path, fold_auc_list, fold_aupr_list, fold_f1_list, fold_acc_list, fold_precision_list, fold_recall_list,fold_specificity_list)
