...
def parse_arguments():
    ...
    parser.add_argument("--config", type=str, default="DAVIS.yaml", help="Path to the config YAML file")#DAVIS.yaml BIOSNAP.yaml DrugBank.yaml
...
def load_dataset(data_path, top_k_d, top_k_p):
    ...
    drug_id_to_embedding = {
        drug_id: drug_smiles_to_embedding[seq]
        for seq, drug_id in drug_id_mapping.items()
        if seq in drug_smiles_to_embedding
    }
    ...
    if os.path.exists(drug_graph_cache_path):
        ...
        drug_similarity_graph = save_dgl_graph(
            drug_graph_cache_path, similarity_matrix, drug_ids, drug_id_to_embedding, top_k=top_k_d
        )
    ...
    prot_id_to_embedding = {
        prot_id: protein_sequences_to_embedding[seq]
        for seq, prot_id in prot_id_mapping.items()
        if seq in protein_sequences_to_embedding
    }
    ...
    if os.path.exists(prot_graph_cache_path):
        ...
        print("Creating protein similarity graph from STRING dataset...")
        prot_similarity_graph = save_dgl_graph_from_links_final(
            prot_graph_cache_path, prot_link_path, prot_ids, prot_id_to_embedding, top_k=top_k_p
        )

    # 内存清理
    del drug_smiles, drug_embeddings, drug_id_mapping_df, protein_sequences, protein_embeddings, prot_id_mapping_df
    del drug_smiles_to_embedding, drug_id_mapping, drug_id_to_embedding
    del protein_sequences_to_embedding, prot_id_mapping, prot_id_to_embedding
    gc.collect()
...
def get_dataloaders(data_path, top_k_d, top_k_t, max_atom_nodes, max_drug_nodes, max_prot_nodes, batch_size,
                    num_workers):
    # 调用你刚才改好的 非5折 加载函数
    train_df, valid_df, test_df, drug_similarity_graph, prot_similarity_graph = load_dataset(
        data_path, top_k_d, top_k_t
    )

    # 使用普通版 CustomDataset
    train_dataset = CustomDataset(train_df, drug_similarity_graph, prot_similarity_graph, max_atom_nodes,
                                  max_drug_nodes, max_prot_nodes)
    train_dataloader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True, num_workers=num_workers,
                                  collate_fn=collate_fn)

    val_dataset = CustomDataset(valid_df, drug_similarity_graph, prot_similarity_graph, max_atom_nodes, max_drug_nodes,
                                max_prot_nodes)
    val_dataloader = DataLoader(val_dataset, batch_size=batch_size, shuffle=False, num_workers=num_workers,
                                collate_fn=collate_fn)

    test_dataset = CustomDataset(test_df, drug_similarity_graph, prot_similarity_graph, max_atom_nodes, max_drug_nodes,
                                 max_prot_nodes)
...
def collate_fn(batch):
    mocular_graphs, drug_features, drug_graphs, prot_sequences, prot_features, prot_graphs, drug_id, prot_id, dg_index, pg_index, labels = zip(
        *batch)
    ...
    prot_seq_features = torch.stack(
        [torch.from_numpy(label_sequence(prot_sequence, CHARPROTSET, 1000)).long() for prot_sequence in prot_sequences],
        dim=0)
...
class CustomDataset(Dataset):
    def __init__(self, df, drug_graph, prot_graph, max_atom_nodes, max_drug_nodes, max_prot_nodes):
